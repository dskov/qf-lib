...
# Setpath is set to no, in order to put the GTK3+ at the beginning of path manually
Start-Process gtk3-runtime-3.22.21-2017-09-25-ts-win64.exe -ArgumentList @('/S', '/setpath=no', '/D=C:\Requirements\GTK3-Runtime Win64') -Wait

# Update the PATH manually with the path to GTK3 bin directory
[Environment]::SetEnvironmentVariable('PATH', 'C:\Requirements\GTK3-Runtime Win64\bin;' + $env:PATH, [EnvironmentVariableTarget]::Machine)
